...
//Función para validar que las palabras ingresadas tengan entre 4 y 15 letras, y no posean caracteres especiales
function validarEntrada(entradas) {
    var palabraInvalida = false;
    if (entrada.length != 0) {
        entrada = entradas.split(" ");
        for (var i = 0; i < entrada.length; i++) {
            if (entrada[i].length < 4 || entrada[i].length > 15) {
